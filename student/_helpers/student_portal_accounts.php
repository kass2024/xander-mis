 ...
 * Keep this here so we don't have to modify any existing config files.
 */
const XGS_STUDENT_DEFAULT_PASSWORD = 'Xander@2026';

function xgs_student_email_norm(string $email): string
{
 ...
 * - If account exists, resets password_hash and forces status='active' (default-password login flow).
 */
function xgs_student_portal_ensure_account_for_email(mysqli $conn, string $email, string $defaultPassword = XGS_STUDENT_DEFAULT_PASSWORD): void
{
    xgs_student_portal_ensure_schema($conn);

    $email = xgs_student_email_norm($email);
